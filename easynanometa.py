...
def adapters_removal(file_path, file_prefix, output_folder, num_threads):
    adapters_removal_out_folder = os.path.join(output_folder, "adapters_removal_out")
    
    command = f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate porechop_abi && porechop_abi --ab_initio -i {file_path} -o {adapters_removal_out_folder}/{file_prefix}_output.fastq -t {num_threads}\""
    try:
...
def host_removal(host_removal_reference, num_threads):
    ...
    for fastq_file in os.listdir(adapters_removal_out_folder):
        if fastq_file.endswith("_output.fastq"):
            ...
            fastq_to_fasta_cmd = (
                f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate base && "
    ...
    minimap2_index_cmd = f"singularity exec -B {hostremoval_reference_directory}:/mnt easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate host_removal && minimap2 -d {host_removal_out_folder}/{reference_prefix}.min /mnt/{fasta_file_name}\""
    try:
    ...
    for fasta_file in fasta_files:
        ...
        minimap2_align_cmd = f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate host_removal && minimap2 -ax map-ont -t {num_threads} {host_removal_out_folder}/{reference_prefix}.min {fasta_file} -o {host_removal_out_folder}/{file_prefix}_minimap.sam\""
        try:
        ...
        extract_unmaped_cmd = f"singularity exec -B {hostremoval_reference_directory}:/mnt easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate host_removal && samtools view -bS -@ {num_threads} -T /mnt/{fasta_file_name} -f 4 {host_removal_out_folder}/{file_prefix}_minimap.sam > {host_removal_out_folder}/{file_prefix}_unmaped_minimap.bam\""
        try:
        ...
        sort_bam_cmd = f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate host_removal && samtools sort -n {host_removal_out_folder}/{file_prefix}_unmaped_minimap.bam -o {host_removal_out_folder}/{file_prefix}_unmaped_sorted_minimap.bam\""
        try:
        ...
        bam_to_fastq_cmd = f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate host_removal && bedtools bamtofastq -i {host_removal_out_folder}/{file_prefix}_unmaped_sorted_minimap.bam -fq {host_removal_out_folder}/{file_prefix}_fitted_raw.fastq\""
        try:
...
def centrifuge(centrifuge_db, num_threads):
    ...
    for fastq_file in os.listdir(host_removal_out_folder):
        if fastq_file.endswith("_fitted_raw.fastq"):
            ...
            centrifuge_cmd = (
                f"singularity exec -B {centrifuge_db_directory}:/mnt easynanometa.sif /tools/Software/centrifuge-1.0.4/bin/centrifuge "
            ...
            centrifuge_to_kraken_cmd = (
                f"singularity exec -B {centrifuge_db_directory}:/mnt easynanometa.sif /tools/Software/centrifuge-1.0.4/bin/centrifuge-kreport "
                f"-x /mnt/{db_name} "
...
def kraken2(kraken2_db, num_threads):
    ...
    for fastq_file in os.listdir(host_removal_out_folder):
        if fastq_file.endswith("_fitted_raw.fastq"):
            ...
            kraken2_cmd = (
                f"singularity exec -B {kraken2_db}:/mnt easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate base && /tools/Software/kraken2-2.1.3/kraken2 "
                f"--db /mnt "
...
def abricate(num_threads):
    ...
    for fasta_file in fasta_files:
        ...
        abricate_arg_cmd = (
            f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate abricate && abricate "
    ...
    for ncbi_result_file in os.listdir(folders["arg"]):
        if ncbi_result_file.endswith("_ncbi_result"):
            ...
            abundance_calculate_cmd = (
                f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate base && python "
...
def metaflye(num_threads):
    ...
    for fastq_file in os.listdir(host_removal_out_folder):
        if fastq_file.endswith("_unique.fastq"):
            ...
            metaflye_cmd = (
                f"singularity exec easynanometa.sif /tools/Software/Flye-2.9.2/bin/flye "
...
def semi_bin(num_threads):
    ...
    for folder_path, prefix in prefixes:
        ...
        for fasta_file in fasta_files: 
            semi_bin_cmd = (
                f"singularity exec easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate SemiBin && "
        ...
        if os.path.isdir(folder_path) and folder_name.endswith("_bin_out"):
            ...
            for subfolder_name in os.listdir(folder_path):
                ...
                if os.path.isdir(output_bins_folder) and subfolder_name == "output_bins":
                    ...
                    checkm2_cmd = (
                        f"singularity exec -B {checkm2_db_directory}:/mnt easynanometa.sif /bin/bash -c \"source /tools/Miniconda3/bin/activate checkm2 && "
...
def main():
    ...
    for file_prefix, file_path in fastq_files.items():
        adapters_removal(file_path, file_prefix, output_folder, args.threads)
